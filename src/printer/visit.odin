		...
		if (.Enforce_Newline in options) {
			document = cons(
				...
				cons_with_nopl(
					...
					cons_with_nopl(
						cons(
							repeat_space(name_alignment - get_node_length(field.name)),
...
visit_stmt :: proc(
	...
	case ^ast.Using_Stmt:
		document = cons(
			document,
			cons_with_nopl(text("using"), visit_exprs(p, v.list, {.Add_Comma})),
		)
	case ^ast.Block_Stmt:
	...
	case ^ast.Return_Stmt:
		...
		if is_values_return_stmt_callable(v.results) {
			...
			if is_return_stmt_ending_with_comp_lit_expr(v.results) {
				...
				document = cons_with_nopl(
					document,
					group(nest(visit_exprs(p, v.results, {.Add_Comma, .Group}))),
				)
			} else {
...
visit_where_clauses :: proc(p: ^Printer, clauses: []^ast.Expr) -> ^Document {
	...
	return nest(
		cons_with_nopl(text("where"), visit_exprs(p, clauses, {.Add_Comma, .Enforce_Newline})),
	)
}
...
visit_expr :: proc(
	...
	case ^ast.Class_Type:
		...
		if v.fields != nil && v.inherits == nil && len(v.fields.list) == 0 {
			if called_from == .Generic {
				document = cons(document, text("{"))
			} else {
				document = cons_with_nopl(document, text("{"))
	...
	case ^ast.Proc_Lit:
		...
		document = cons_with_nopl(
			document,
			visit_proc_type(p, v.type^, v.body != nil, len(v.where_clauses) > 0),
		)
	...
	case ^ast.Call_Expr:
		...
		if is_call_expr_nestable(v.args) {
			...
			document = cons(
				document,
				nest_if_break(cons(break_with(""), visit_call_exprs(p, v)), "call_expr"),
			)
		}
	...
	case ^ast.Selector_Expr:
		document = enforce_fit(
			cons(visit_expr(p, v.expr), text_token(p, v.op), visit_expr(p, v.field)),
		)
	case ^ast.Paren_Expr:
	...
	case ^ast.Comp_Lit:
		...
		if v.type != nil {
			...
			if matrix_type, ok := v.type.derived.(^ast.Matrix_Type);
			   ok &&
			   is_matrix_type_constant(matrix_type) &&
			   is_matrix_filled_comp_lit(matrix_type, v) {
				...
				document = cons(
					...
					nest(
						cons(
							...
							visit_matrix_comp_lit(p, matrix_type, v),
						),
					),
				)
				set_source_position(p, v.end)
...
visit_class_inheritance :: proc(
	...
	for base, i in list {
		...
		document = cons(
			document,
			cons_with_nopl(
				cons_with_nopl(text("using"), cons(text("base___"), name)),
...
visit_proc_type :: proc(
	...
	if proc_type.results != nil && len(proc_type.results.list) > 0 {
		...
		if use_parens {
			...
			results_no_parens := nest(
				group(visit_signature_list(p, proc_type.results, contains_body, true)),
			)
			if can_multiline_single {
				document = cons_with_nopl(
					document,
					if_break_or_document(results_parens, results_no_parens),
				)
			} else {
