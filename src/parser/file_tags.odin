...
@(require_results)
get_build_os_from_string :: proc(
	str: string,
) -> (
	found_os: runtime.Odin_OS_Type,
	found_subtarget: runtime.Odin_Platform_Subtarget_Type,
) {
			...
			case "build":
				...
				kinds_loop: for {
					...
					defer append(
						build_kinds,
						Build_Kind {
							os = (os_positive == {} ? runtime.ALL_ODIN_OS_TYPES : os_positive) -
							os_negative,
							arch = (arch_positive == {} ? runtime.ALL_ODIN_ARCH_TYPES : arch_positive) -
							arch_negative,
						},
					)
					...
					for {
						...
						if value == "ignore" {
							tags.ignore = true
						} else if os, subtarget := get_build_os_from_string(value);
						   os != .Unknown {
